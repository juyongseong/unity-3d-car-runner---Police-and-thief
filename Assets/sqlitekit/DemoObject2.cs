...
public class DemoObject2 : MonoBehaviour {
	...
	void OnGUI()
	{
		if( demoData == null ) 
		{
			if ( GUI.Button(new Rect (10,10,150,70), "Run asynchronous") ) 
			{
				...
				asyncDB.Query(queryInsert, InsertBind, InsertComplete, demoData);
			}
		}
	...
	void InsertComplete(SQLiteQuery qr, object state)
	{
		// call step asynchronously 
		asyncDB.Step(qr, null, InsertStepInvoked, state);
	}

	
	void InsertStepInvoked(SQLiteQuery qr, object state)
	{
	...
	void InsertQueryReleased(object state)
	{
		...
		{
			// ok, add more please :)
			asyncDB.Query(queryInsert, InsertBind, InsertComplete, data); 
		}
		else
		{
			// we just added 1000 records, now read them!
			asyncDB.Query("SELECT * FROM test_values", null, SelectQueryCreated, data);
		}
	}
	...
	void SelectQueryCreated(SQLiteQuery qr, object state)
	{
		...
		asyncDB.Step(qr, SelectStepCallback, SelectComplete, state);
	}
	...
	void SelectComplete(SQLiteQuery qr, object state)
	{
